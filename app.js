...
function cambiarMacroCategoria(idCategoria) {
    ...
    dataCategoria.subtopics.forEach(subtopic => {
        ...
        if (subtopic.name.toLowerCase().includes("constituc")) iconoSubtema = "📖";
...
function armarTarjeta(perfilData, configCand, nombreSubtema) {
    ...
    let htmlCita = "";
    if (perfilData.reciente) {
        htmlCita = `<p class="quote-text">"${perfilData.reciente}"</p>
        <div class="meta-data">
            🗓 Emitido: ${perfilData.fecha_reciente} | <a href="${perfilData.fuente_reciente}" target="_blank">Ver fuente</a>
        </div>`;
    } else {
    ...
    if (
        ...
        alertaCambioHtml = `<div class="alerta-cambio">🚨 Evidencia de Cambio de Postura</div>`;
        autoAbrir = true;
    }
